...
def run_fig3():
    ...
    patterns = [
        {"fgds_regmem(ns)": r"fgds_regmem:\s*(\d+)\s*ns",
        "fgds_deregmem(ns)": r"fgds_deregmem:\s*(\d+)\s*ns"},
...
def run_table3():
    ...
    patterns = [
        {"fgds_open(ns)": r"fgds_open:\s*(\d+)\s*ns",
        "fgds_close(ns)": r"fgds_close:\s*(\d+)\s*ns"},
...
def run_fig6():
    ...
    final_result = pd.concat(result, 
    axis=1,
    keys=["fgds", "NVIDIA GDS", "fgds with stream"])
    final_result["block size(KB)"] = block_size
    final_result.set_index("block size(KB)", inplace=True)
...
def run_fig8():
    ...
    for t in [0, 1]:
        ...
        for bs in [GB, 2 * GB, 4 * GB]:
            data = run_cmd(bin_path, [file_path, bs, "fgds" if t == 0 else "gds"])
            if only_get_command:
                continue
            data = data.split("\n")[-3:-1]
            tmp = []
...
def run_fig11():
    ...
    for trace in trace_text:
        for t in [0, 1]:
            trace_result = []
            for b in block_size:
                data = run_cmd(bin_path, ["fgds" if t == 0 else "gds", 0, os.path.join(traces_path, trace), b, file_path])
                if only_get_command:
                    continue
