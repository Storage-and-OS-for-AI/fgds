...
def get_device_id(dest_device: str = "cuda") -> int:
    device_id = 0
...
class FgdsMemoryAllocator(GPUMemoryAllocator):
    def __init__(self, size: int, device: Optional[str] = None) -> None:
        # HACK: fgds import may be buggy on some hardware
        # (e.g., without GPUDirect), so it's temporarily put here.
        # Third Party
        from fgds import fgds_deregmem, fgds_regmem
        ...
        logger.info("FgdsMemoryAllocator, device=%d", self.device)
    ...
    def __del__(self) -> None:
        if hasattr(self, "base_pointer") and hasattr(self, "fgdsBufDeReg"):
            try:
                self.fgdsBufDeReg(
                    self.device,
                    ctypes.c_void_p(self.base_pointer),
...
def pack_metadata(tensor: torch.Tensor, fmt: MemoryFormat, **extra_metadata) -> bytes:
...
def rand_suffix(n: int) -> str:
    return uuid.uuid4().hex[:n]


async def save_metadata(path: str, tmp: str, metadata: bytes) -> None:
    tmp_path = path + tmp
...
def get_extra_config_bool(key: str, config: LMCacheEngineConfig) -> Optional[bool]:
...
def _load_gpu_memcpy() -> Callable:
    ...
    for lib_name, fallback_path, symbol in candidates:
        try:
            ...
            continue
    raise RuntimeError(
        "FGDS POSIX fallback requires a GPU runtime library "
...
class FgdsBackend(AllocatorBackendInterface):
    ...
    GDS-supported remote filesystem via FGDS library. In order to use it,
    users need to specify `fgds_path` and `fgds_buffer_size` in their
    LMCache config.
    ...
    NOTE: If GPUDirect is not supported on that filesystem, then CuFile will
    ...
    def __init__(
        ...
        if self.fstype in ["tmpfs", "overlayfs"]:
            # TODO: we can replace the auto-detection of unsupported fgds
            # file systems by doing a small fgds API test on them. If a
            # read/write test fails, we can fallback to not using fgds APIs.
            if use_fgds_from_config:
                logger.warning("No automatic disabling of fgds usage due to fstype")
            else:
                logger.info("Automatic disabling of fgds usage due to fstype")
                self.use_fgds = False
        ...
        if self.use_thread_pool:
            self._thread_pool = ThreadPoolExecutor(
                max_workers=thread_count, thread_name_prefix="fgds-io"
            )

        if self.use_fgds:
            logger.info("Using FGDS")
            # HACK: fgds import is buggy on some hardware
            # (e.g., without GPUDirect), so it's temporarily put here.
            # Third Party
            import fgds
            ...
            logger.info("FGDS disabled, using POSIX fallback")
            self.fgds = None
            self._gpu_memcpy = _load_gpu_memcpy()

        self.use_direct_io = True

        # Values for retrying allocations and loads in case of failures
        ...
        # flag to use O_NOATIME during metadata file read for performance
                    ...
                    for fentry in it2:
                        ...
                        try:
                            ...
                            logger.error(
                                ...
                                "ignoring during FGDS start",
                                fentry.path,
                            )
                        except Exception:
                            logger.error(
                                "Failed to read metadata file %s during FGDS start; "
    ...
    def _try_to_read_metadata(self, key: CacheEngineKey) -> Optional[DiskCacheMetadata]:
        ...
        if os.path.exists(path):
            try:
                ...
                logger.warning(
                    "[FGDS] File not found for key %s at expected path %s",
                    key.to_string(),
                    path,
                )
            except PermissionError:
                logger.warning(
                    "[FGDS]: Permission Denied for PID %s on %s",
                    os.getpid(),
                    path,
                )
            except UnsupportedMetadataVersion:
                ...
                logger.error(
                    "Failed to read metadata file %s: %s: %s",
                    path,
                    type(e).__name__,
                    e,
                )
    ...
    async def _async_save_bytes_to_disk(
        ...
        try:
            ...
            if subdir_key not in self.metadata_dirs:
                os.makedirs(
                    os.path.join(self.fgds_path, l1_dir, l2_dir), exist_ok=True
                )
                self.metadata_dirs.add(subdir_key)
            ...
            try:
                ...
                logger.error(
                    "FGDS write operation failed for key %s at path %s: "
    ...
    def _save_gds(
        ...
        try:
            ...
            if self.fgds:
                ...
                raise RuntimeError(
                    "Both fgds and _gpu_memcpy are None, this should not happen"
                )
        except Exception as e:
    ...
    def _load_gds(
        ...
        try:
            ...
            logger.error("FGDS read failed for %s: %s", file_path, e, exc_info=True)
            return -1
    ...
    def remove(self, key: CacheEngineKey, force: bool = True) -> bool:
        raise NotImplementedError("FGDS backend does not support remove now.")
    ...
    def initialize_allocator(
        ...
        return FgdsMemoryAllocator(
            config.fgds_buffer_size * 1024**2, device=self.dst_device
        )
    ...
    def allocate(
        ...
        while True:
            ...
            if num_attempts < max_attempts:
                logger.debug(
                    "Unable to allocate memory object after %s attempt(s) of FGDS "
        ...
        logger.warning(
            "FGDS allocation failed after %s attempt(s). Returning None.",
            num_attempts,
        )
        if not self.memory_allocator.memcheck():
            logger.error(
                "FGDS allocation failed and memory allocator "
    ...
    def batched_allocate(
        ...
        logger.debug(
            "Batched allocating memory in FGDS backend with busy loop: %s",
            busy_loop,
        )
        ...
        logger.warning(
            "FGDS batched allocation failed after %s attempt(s). Returning None.",
            num_attempts,
        )
        if not self.memory_allocator.memcheck():
            logger.error(
                "FGDS batched allocation failed and memory allocator "
                "is inconsistent. This is a bug in the memory allocator."
            )
        return None
