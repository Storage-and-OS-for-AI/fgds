...
typedef struct fgds_mmap_node_s {
    void **vaddrs;
    size_t mmap_count;
    uint64_t n_addr;
    size_t length;
...
typedef struct fgds_mmap_buffer_s {
    int device_id;
    int bdev_fd;
    bool init_stat;
...
static fgds_mmap_buffer_t mbuffer[FGDS_MAX_DEVICES];
...
void free_fgds_p2p_map(fgds_mmap_buffer_t *buffer) {
...
int fgds_close(int device_id) {
    if (device_id >= 0 && device_id < g_device_count) {
        if (fgds_initialized[device_id]) {
            __fgds_close(&mbuffer[device_id]);
...
int fgds_open(int deviceID) {
    int ret;
    if (!is_fgds_initialized()) {
        if (deviceID == -1) {
            for (int id = 0; id < g_device_count; ++id) {
                if (!fgds_initialized[id]) {
                    ret = __fgds_open(fgds_dev_path[id].c_str(), &mbuffer[id]);
                    if (ret < 0) {
                        fgds_close_all();
                        return ret;
                    }
            ...
            if (!fgds_initialized[deviceID]) {
                ret = __fgds_open(fgds_dev_path[deviceID].c_str(), &mbuffer[deviceID]);
                fgds_initialized[deviceID] = true;
                return ret;
            }
        } else { // only start device id = 0
            if (!fgds_initialized[0]) {
                ret = __fgds_open(fgds_dev_path[deviceID].c_str(), &mbuffer[0]);
...
int insert_fgds_mmap_node(fgds_mmap_buffer_t *mbuffer, fgds_p2p_map_t *new_node) {
    if (!new_node) {
...
fgds_p2p_map_t *find_fgds_mmap_node(fgds_mmap_buffer_t *mbuffer, u64 n_addr, u64 len) {
    fgds_p2p_map_t *current = mbuffer->head;
    while (current) {
...
int delete_fgds_mmap_node(fgds_mmap_buffer_t *mbuffer, fgds_p2p_map_t *p2p_map) {
    fgds_p2p_map_t *current = mbuffer->head;
    fgds_p2p_map_t *previous = NULL;
    pthread_mutex_lock(&mbuffer->lock);
    while (current) {
...
static inline int __fgds_regmem(fgds_mmap_buffer_t *mbuffer, u64 n_addr, u64 c_addr, size_t len) {
    ...
    ret = ioctl(mbuffer->bdev_fd, FGDS_IOCTL_MAP, &para);
...
int __fgds_deregmem(fgds_mmap_buffer_t *pb, u64 n_addr, u64 c_addr, size_t len) {
    ...
    ret = ioctl(pb->bdev_fd, FGDS_IOCTL_UNMAP, &para);
...
int fgds_deregmem(int device_id, const void *addr, size_t len) {
    ...
    fgds_p2p_map_t *p2p_map;
    fgds_mmap_buffer_t *pb = &mbuffer[device_id];

    p2p_map = find_fgds_mmap_node(pb, (u64)addr, len);
    if(p2p_map == NULL) {
    ...
    ret = delete_fgds_mmap_node(pb, p2p_map);
    if (ret) {
...
ssize_t fgds_read(fgds_fileid_t fid, void *buf, off_t buf_offset, ssize_t nbyte, off_t f_offset){
    ...
    ssize_t ret, i;
    void *target_addr;
    xfer_addr = fgds_do_xfer_addr(fid.deviceID, buf, buf_offset, nbyte);

    if (xfer_addr == NULL) {
        fprintf(stderr, "%s: fgds_read pread error\n", __func__);
        return -1;
    }
...
ssize_t fgds_write(fgds_fileid_t fid, void *buf, off_t buf_offset, ssize_t nbyte, off_t f_offset){
    ...
    for (i = 0;i < xfer_addr->nr_xfer_addrs; i++){
        ...
        if((size_t)ret != nbyte_per_iter){
            fprintf(stderr, "%s: fgds_read pread error: ret is %d\n", __func__, ret);
            std::free(xfer_addr);
            return -1;
        }
...
struct fgds_xfer_addr *fgds_do_xfer_addr(int device_id, const void *buf, off_t buf_offset, size_t nbyte) {
    struct fgds_xfer_addr *addr;
    fgds_mmap_buffer_t *_local_mbuffer = &mbuffer[device_id];
    fgds_p2p_map_t *p2p_map;
    ...
    p2p_map = find_fgds_mmap_node(_local_mbuffer, (u64)buf, nbyte);
    ...
    addr = (struct fgds_xfer_addr *)std::malloc(sizeof(struct fgds_xfer_addr) + (count - 1) * sizeof(struct xfer_addr));
    if (!addr) {
